...
//  YCSB-cpp
//
//  Copyright (c) 2020 Chao Zhou <zc1012786567@gmail.com>.
//  Modifications Copyright 2023 Chengye YU <zc1012786567@gmail.com>.
//
...
leveldb::DB *BlockdbDB::db_ = nullptr;
int BlockdbDB::ref_cnt_ = 0;
std::mutex BlockdbDB::mu_;
...
void BlockdbDB::Init() {
  ...
  if (format == "single") {
    ...
    method_read_ = &BlockdbDB::ReadSingleEntry;
    method_scan_ = &BlockdbDB::ScanSingleEntry;
    method_update_ = &BlockdbDB::UpdateSingleEntry;
    method_insert_ = &BlockdbDB::InsertSingleEntry;
    method_delete_ = &BlockdbDB::DeleteSingleEntry;
  } else if (format == "row") {
    format_ = kRowMajor;
    method_read_ = &BlockdbDB::ReadCompKeyRM;
    method_scan_ = &BlockdbDB::ScanCompKeyRM;
    method_update_ = &BlockdbDB::InsertCompKey;
    method_insert_ = &BlockdbDB::InsertCompKey;
    method_delete_ = &BlockdbDB::DeleteCompKey;
  } else if (format == "column") {
    format_ = kColumnMajor;
    method_read_ = &BlockdbDB::ReadCompKeyCM;
    method_scan_ = &BlockdbDB::ScanCompKeyCM;
    method_update_ = &BlockdbDB::InsertCompKey;
    method_insert_ = &BlockdbDB::InsertCompKey;
    method_delete_ = &BlockdbDB::DeleteCompKey;
  } else {
  ...
  if (db_path == "") {
    throw utils::Exception("BlockDB db path is missing");
  }
  else
  {
    std::cout << "BlockDB db path: " << db_path << std::endl;
  }
...
void BlockdbDB::GetOptions(const utils::Properties &props, leveldb::Options *opt) {
...
void BlockdbDB::GetWriteOptions(const utils::Properties &props, leveldb::WriteOptions *write_opt)
{  
...
void BlockdbDB::SerializeRow(const std::vector<Field> &values, std::string *data) {
...
void BlockdbDB::DeserializeRowFilter(std::vector<Field> *values, const std::string &data,
...
void BlockdbDB::DeserializeRow(std::vector<Field> *values, const std::string &data) {
  const char *p = data.data();
  const char *lim = p + data.size();
  while (p != lim) {
    assert(p < lim);
...
std::string BlockdbDB::BuildCompKey(const std::string &key, const std::string &field_name) {
  switch (format_) {
    case kRowMajor:
...
std::string BlockdbDB::KeyFromCompKey(const std::string &comp_key) {
  size_t idx = comp_key.find(":");
  assert(idx != std::string::npos);
  return comp_key.substr(0, idx);
}

std::string BlockdbDB::FieldFromCompKey(const std::string &comp_key) {
  size_t idx = comp_key.find(":");
  assert(idx != std::string::npos);
  return comp_key.substr(idx + 1);
}

DB::Status BlockdbDB::ReadSingleEntry(const std::string &table, const std::string &key,
                                      const std::vector<std::string> *fields,
                                      std::vector<Field> &result) {
  std::string data;
  leveldb::Status s = db_->Get(leveldb::ReadOptions(), key, &data);
  if (s.IsNotFound()) {
    std::cout << "zc BlockdbDB::ReadSingleEntry::db_->Get not found" << std::endl;
    return kNotFound;
  } else if (!s.ok()) {
...
DB::Status BlockdbDB::ScanSingleEntry(const std::string &table, const std::string &key, int len,
                                      const std::vector<std::string> *fields,
...
DB::Status BlockdbDB::UpdateSingleEntry(const std::string &table, const std::string &key,
                                        std::vector<Field> &values) {
  std::string data;
  leveldb::Status s = db_->Get(leveldb::ReadOptions(), key, &data);
  if (s.IsNotFound()) {
    std::cout << "zc BlockdbDB::UpdateSingleEntry::db_->Get not found" << std::endl;
    return kNotFound;
  } else if (!s.ok()) {
    throw utils::Exception(std::string("LevelDB Get: ") + s.ToString());
  }
  std::cout << "zc BlockdbDB::UpdateSingleEntry::db_->Get found" << std::endl;
...
DB::Status BlockdbDB::InsertSingleEntry(const std::string &table, const std::string &key,
                                        std::vector<Field> &values) {
  std::string data;
...
DB::Status BlockdbDB::DeleteSingleEntry(const std::string &table, const std::string &key) {
  leveldb::WriteOptions wopt;
...
DB::Status BlockdbDB::ReadCompKeyRM(const std::string &table, const std::string &key,
                                    const std::vector<std::string> *fields,
                                    std::vector<Field> &result) {
  leveldb::Iterator *db_iter = db_->NewIterator(leveldb::ReadOptions());
  db_iter->Seek(key);
...
DB::Status BlockdbDB::ScanCompKeyRM(const std::string &table, const std::string &key, int len,
                                    const std::vector<std::string> *fields,
                                    std::vector<std::vector<Field>> &result) {
  leveldb::Iterator *db_iter = db_->NewIterator(leveldb::ReadOptions());
  db_iter->Seek(key);
...
DB::Status BlockdbDB::ReadCompKeyCM(const std::string &table, const std::string &key,
                                    const std::vector<std::string> *fields,
                                    std::vector<Field> &result) {
  return kNotImplemented;
}

DB::Status BlockdbDB::ScanCompKeyCM(const std::string &table, const std::string &key, int len,
                                    const std::vector<std::string> *fields,
                                    std::vector<std::vector<Field>> &result) {
  return kNotImplemented;
}

DB::Status BlockdbDB::InsertCompKey(const std::string &table, const std::string &key,
                                    std::vector<Field> &values) {
  leveldb::WriteOptions wopt;
...
DB::Status BlockdbDB::DeleteCompKey(const std::string &table, const std::string &key) {
  leveldb::WriteOptions wopt;
  leveldb::WriteBatch batch;
...
void BlockdbDB::PrintStats() { 
    std::string stats;
    if(nullptr == db_)
    {
      throw utils::Exception("BlockdbDB::PrintStats failed: db has been deleted.");
      return;
    }
    ...
    {
      throw utils::Exception("BlockdbDB::PrintStats failed: there is an issue with the native interface.");
    }

}

DB *NewBlockdbDB() {
  return new BlockdbDB;
}

const bool registered = DBFactory::RegisterDB("blockdb", NewBlockdbDB);
