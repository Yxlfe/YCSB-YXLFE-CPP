...
#ifndef YCSB_C_BLOCKDB_DB_H_
#define YCSB_C_BLOCKDB_DB_H_
...
#include <blockdb/db.h>
#include <blockdb/options.h>
#include <blockdb/status.h>
#include <blockdb/cache.h>
#include <blockdb/filter_policy.h>
#include <blockdb/write_batch.h>

namespace ycsbc {

class BlockdbDB : public DB {
 public:
  BlockdbDB() {}
  ~BlockdbDB() {
    if (nullptr != db_)
 ...
 private:
  ...
  void GetWriteOptions(const utils::Properties &props, leveldb::WriteOptions *write_opt);
  ...
  Status (BlockdbDB::*method_read_)(const std::string &, const std:: string &,
                                    const std::vector<std::string> *, std::vector<Field> &);
  Status (BlockdbDB::*method_scan_)(const std::string &, const std::string &, int,
                                    const std::vector<std::string> *,
                                    std::vector<std::vector<Field>> &);
  Status (BlockdbDB::*method_update_)(const std::string &, const std::string &,
                                      std::vector<Field> &);
  Status (BlockdbDB::*method_insert_)(const std::string &, const std::string &,
                                      std::vector<Field> &);
  Status (BlockdbDB::*method_delete_)(const std::string &, const std::string &);
