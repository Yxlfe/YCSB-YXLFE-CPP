...
class DBWrapper : public DB {
 public:
  DBWrapper(DB *db, Measurements *measurements) : DB(db->GetProps()), db_(db), measurements_(measurements) {}
  ~DBWrapper() {
    delete db_;
  }
  void Init() {
  ...
  utils::Properties * GetProps() const{
    return db_->GetProps();
  }
 ...
 private:
  ...
  utils::Timer<uint64_t, std::micro> timer_;
};
